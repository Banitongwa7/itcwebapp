...
const ItemLog = ({item}) => {
  ...
  return (
    <li class="mb-10 ml-6">
      ...
      { item.info == "ERROR" ? (<h3 class="mb-1 text-md font-semibold text-red-800">{item.info}</h3>) : (<h3 class="mb-1 text-md font-semibold text-gray-800">{item.info}</h3>)}
      <time class="block mb-2 text-sm font-medium leading-none text-sky-800">Détecté le {date} à {time}</time>
      <p class="text-base font-normal text-gray-700">{item.message}</p>
    </li>
  )
}
